...
import pandas as pd
from functions import get_connection, resolve_card, create_sale_tiles
from st_keyup import st_keyup
import math

con = get_connection()
sales = create_sale_tiles(con)
...
sales= sales[sales['cardName'].str.contains(current_filter, case=False)]
if set_filt is not None:
    sales = sales[sales['setName'] == set_filt]
...
sales = sales.sort_values(by=[sort_column], ascending=asc_desc)

# Paginate the sales grid
page_size = 40
total_pages = max(1, math.ceil(len(sales) / page_size))
page = st.number_input('Page', min_value=1, max_value=total_pages, value=1, step=1)
start = (page - 1) * page_size
page_sales = sales.iloc[start:start + page_size]
st.caption(f'Showing {start + 1}-{min(start + page_size, len(sales))} of {len(sales)} cards')

with st.container(height = 650, ):
    cols = st.columns(4)
    for i, (_, card) in enumerate(page_sales.iterrows()):
        col = cols[i % 4]
        if i % 4 == 0 and i != 0:
            cols = st.columns(4)
            col = cols[0]
